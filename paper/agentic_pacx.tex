...
Beyond raw detection accuracy, cloud security operations require verifiable trust: an analyst must audit why a request was flagged, and the system must adapt safely as new malware families emerge without manual retraining. Agentic-PACX addresses both directly: an LLM-driven trust arbitration step names which pathway to trust on disagreement, a second LLM agent gates a guardrailed self-healing retraining routine, and a fail-closed escalation rule resolves uncertainty about novel threats toward caution rather than false clearance. The remainder of this paper is organized as follows: Section II reviews related work, Section III details the methodology, Section IV presents results, and Section V concludes.

\section{Related Work}
Prior API-call analysis spans system API vectorization \cite{b6}, API2Vec++ sequence boosting \cite{b15}, \cite{b16}, and deep networks over API sequences \cite{b18}, \cite{b17}. Mobile/IoT work uses calls, permissions, and opcode features \cite{b8}, \cite{b10}, \cite{b11}, plus genetic-algorithm-optimized networks \cite{b13}. Beyond malware analysis, AI-enhanced API gateways secure microservices \cite{b5}, alongside malicious-IP identification \cite{b4}, network-level hybrid deep learning \cite{b1}, cloud-storage EDoS defenses \cite{b3}, and federated IoT security \cite{b9}. As adversaries evolve obfuscated malware against static and AI-driven detection \cite{b7}, structural approaches follow: fuzzy-hashing CNNs for ransomware \cite{b12}, and GNNs via contextual steerable node localization \cite{b2} or graph attention for code localization \cite{b14}.

The evolution of Explainable AI (XAI) in cybersecurity motivates our research. A pivotal advancement is the PAC-X framework \cite{b19}: a Conditional Attention Neural Network (CAN-Net) with Contextual Fuzzy Clustering (CFC), reported to outperform LIME, LEMNA, and SHAP on explainability. Agentic-PACX builds on this interpretability philosophy but targets cloud environments via a dual-agent architecture: a GAT for structural dependencies and an LLM-driven fusion framework for natural language explanations. Table \ref{tab:limitations} contrasts Agentic-PACX against the five methodologies most structurally comparable to our own -- spanning fuzzy-explainable classification, static graph construction, genetic-hyperparameter search, sequence embedding, and post-hoc local explanation -- listing each approach's core limitation alongside the specific mechanism in our architecture that addresses it.
...
\hline
\textbf{PAC-X (CAN-Net) \cite{b19}} & A neural classifier over flat tabular features; its fuzzy-clustering (CFC) layer degrades under concept drift. & Adds structural GAT attention over graph-topological semantics, plus Agent 2 retraining to counter drift. \\
\hline
\textbf{CNN-DNN Hybrid \cite{b17}} & Builds its API graph via static decompilation (Apktool/smali), inheriting static analysis's vulnerability to obfuscation. & Computes GAT attention over a lightweight 4-node graph at inference, without decompilation. \\
\hline
\textbf{GA CNN-SENet \cite{b13}} & Its genetic-algorithm search is a batch, offline procedure with no online adaptation, and generalization beyond its fixed 8-class set is unreported. & Deploys Agent 2 for autonomous, real-time self-healing on low-confidence zero-day events, evaluated on held-out samples. \\
\hline
\textbf{API2Vec++ \cite{b15}} & Requires per-sample graph construction and random-walk generation before BERT embedding, adding latency. & Uses lightweight 4-node PyG topological graphs achieving high accuracy with minimal inference latency. \\
\hline
\textbf{API-MalDetect \cite{b18}} & Black-box deep learning output with opaque LIME approximations that lack structural context. & Employs Agent 1 (Forensic LLM) to generate XAI reports directly from edge attention weights. \\
\hline
\end{tabular}%
}
\end{center}
...
% 3. Agent 1 (trust scoring) feeds the Fusion Layer -- Agent 1 runs first
% since fusion's disagreement branch (Eq. 9) needs its trust scores as input.
\path (pacx.south) -- (gnn.south) coordinate[midway] (path_mid);

\node[proc, below=0.45cm of path_mid, text width=5.2cm] (agent1) {
    \textbf{Agent 1: Trust Scoring}\\
    Evidential $\text{Trust}_{\text{PAC-X}}$ / $\text{Trust}_{\text{GNN}}$ from confidence + evidence
};

\node[proc, below=0.35cm of agent1, text width=5.2cm] (fusion) {
    \textbf{Decision Fusion Layer}\\
    Agree: weighted blend $w_{\text{PAC-X}} S_{\text{PAC-X}} + w_{\text{GNN}} S_{\text{GNN}}$\\
    Disagree: arbitrate by Agent 1's trust
};

% 4. Condition & Agent 2 / Output
\node[dia, below=0.35cm of fusion] (cond) {Is $C_{\text{final}} < \GateTau$?};

\node[proc, left=2.3cm of cond] (out) {
    \textbf{Output \& Visualization}\\
    Malicious/Benign Prediction\\
    Human-Readable XAI Report
};

\node[proc, right=2.3cm of cond] (agent2) {
...
\draw[arr] (pacx.south) |- ([yshift=0.2cm]agent1.north) -- (agent1.north);
\draw[arr] (gnn.south) |- ([yshift=0.2cm]agent1.north) -- (agent1.north);

\draw[arr] (agent1) -- node[right=1pt, font=\tiny] {trust\_scores} (fusion);
\draw[arr] (fusion) -- (cond);

\draw[arr] (cond.west) -- node[above=3pt, font=\scriptsize, midway, align=center] {No\\(High Conf)} (out.east);
\draw[arr] (cond.east) -- node[above=3pt, font=\scriptsize, midway, align=center] {Yes\\(Zero-Day)} (agent2.west);
...
\end{aligned}
\end{equation}
with $\hat{s} = \text{ABISuffix}(\text{SysNorm}(\text{LibRef}(s)))$. This canonicalization defined by Eq.~1 is the adapter's deliberately approximate bridge from unstructured logs into the model's 4-node space, since free text cannot supply the structured fields the model was trained on (Section III-B.2).
...
Pathway weights start from a 0.6/0.4 GNN/PAC-X baseline, nudged by completeness and certainty (Eq.~\ref{eq:fusioninputs}; $w_{\text{PAC-X}} = 1 - w_{\text{GNN}}$). On agreement, the fused probability is their weighted blend; on disagreement, the layer arbitrates per-sample by trust instead of averaging the disagreement away:
\begin{equation}
\label{eq:fusion}
\begin{aligned}
...
The fused label is an $\arg\max$ over families the network was \emph{trained} on, so a novel family is absorbed by the nearest known class -- and since the benign cluster is largest and least structurally distinctive, overwhelmingly ``benign''. A zero-day is by definition an attack, so this is the dangerous failure direction. These samples already carry fused confidence below the Agent 2 gate $\tau$, but a hard $\arg\max$ discards that signal; we instead reuse it as a decision rule, giving a third verdict state:
\begin{equation}
\label{eq:failclosed}
\hat{y}_{\text{final}} =
\begin{cases}
...
\end{cases}
\end{equation}
where $\tau = \GateTau$ (Section~\ref{sec:agent2}). Only a \emph{benign} verdict escalates, since a low-confidence \emph{malicious} one is already actioned; the state is ``Suspicious'' rather than ``Malicious'' because the system failed to \emph{clear} the sample, not identified an attack; and as a labelling policy it changes no weights, so it cannot regress the model. This composes the reject-option formulation \cite{b20} with the fail-closed principle standard in security: abstention resolves toward denial.
...
\end{aligned}
\end{equation}
where $E_{\text{PAC-X}}$ and $E_{\text{GNN}}$ denote empirical evidence scores derived from detected API frequencies and attention magnitudes, $p_{\text{fam}}$ represents family classification confidence, and $\text{Acc}$ is the deployed model's own measured test accuracy (Section IV-C) -- a fixed, per-model-version prior distinct from $p_{\text{raw}}$'s per-sample confidence. Agent 1 primes an LLM with the attention matrix $\boldsymbol{\alpha}_{ij}$ to produce XAI reports.

\subsubsection{Autonomous Retraining (Agent 2)}
\label{sec:agent2}
If fused confidence falls below $\tau = \GateTau$, Agent 2 pools the sample for an active self-healing transfer-learning routine (Algorithm \ref{alg:agent2}). The gate is not a round-number default: as Fig.~\ref{fig:tausweep} shows, sweeping $\tau$ across all ten leave-one-family-out runs, $\GateTau$ sits just below the point where false positives rise sharply for diminishing catch-rate gains. The same $\tau$ governs the fail-closed rule of Section~\ref{sec:failclosed}, so one threshold carries one meaning: ``the system cannot vouch for this sample.''
...
The framework was evaluated on a PE malware dataset of \DatasetTotal{} graph instances across 11 classes: Benign and 10 named malware families (Gandcrab, Emotet, Gamarue, Hotbar, DarkKomet, Delf, Domaiq, DLHelper, DriverPack, GameHack), sourced from the public GitHub repository accompanying \cite{b19} (verified byte-identical to \texttt{final\_data.csv}). Four feature groups feed the graph: (1) PE Header structures ($v_0$), (2) section-entropy statistics ($v_1$), (3) import/export list embeddings ($v_2$), and (4) network/string artifact embeddings ($v_3$) -- Section III-B.2 details why all four are real numeric blocks, not hashed tokens. Each node is a 260-dimensional feature vector.

The dataset was partitioned using a 70/10/20 stratified train/validation/test split (seeded per-class for reproducibility): \TrainCount{} training, \ValCount{} validation, and \TestCount{} held-out test samples. The validation split -- not the test split -- drives early stopping and checkpoint selection, so the test split is touched exactly once, after model selection, avoiding the bias of selecting a checkpoint on the same set its final number is reported against.
...
Standard metrics are used throughout, where $TP, TN, FP, FN$ denote True/False Positives/Negatives:
\begin{equation}
\label{eq:accuracy}
...
\end{equation}
For class imbalance across the 11 families, unweighted macro-average F1 is used:
\begin{equation}
\label{eq:f1macro}
...
Agentic-PACX achieves \BinaryAcc{} binary detection accuracy with a lightweight 4-node PyG graph, without API2Vec++'s random-walk generation or API-MalDetect's LIME step -- second only to PAC-X's 99.83\% figure (different dataset/protocol). Its differentiating contribution is lightweight inference plus autonomous LLM reasoning and self-healing retraining, which non-agentic baselines lack.

To evaluate the Fusion Layer's contribution rather than assume it, we ran an ablation isolating each pathway on the same held-out set: GNN-only reaches \AblGnnAcc{} accuracy (\AblGnnPrec{} prec., \AblGnnRec{} recall), while PAC-X on this dataset's PE-header CSV serialization -- its weaker modality, with no raw API-log input here -- collapses to the majority (Benign) class on all \TestCount{} samples, \AblPacxAcc{} accuracy with 0\% precision/recall on the malicious class. The fused output matches GNN-only element-for-element: on disagreement the layer arbitrates by trust (Eq.~\ref{eq:fusion}, computed per Eqs.~\ref{eq:trustpacx}--\ref{eq:trustgnn}), and PAC-X's $S_{\text{PAC-X}}$ score (Eq.~\ref{eq:pacx}) collapses to near-zero on unfamiliar tabular input, so the GNN wins essentially every disagreement -- the layer contributes safe arbitration rather than an accuracy lift, as expected given PAC-X's intended raw API-log modality.

\subsection{Explainability and Zero-Day Adaptation}
A major advantage of MalwareGAT is its ability to expose learned attention weights $\alpha_{ij}$ (Eq.~\ref{eq:softmax}), normalized from the raw attention scores $e_{ij}$ (Eq.~\ref{eq:attn}) and consumed by the message-passing update of Eq.~\ref{eq:gatupdate}, indicating relative importance during message passing. Agent 1 ingests these topological weights to generate natural-language forensic explanations of the flagged structural anomaly.
...
The evaluations above measure accuracy on \emph{known} families. To test generalization to a genuinely unseen threat, we ran a leave-one-family-out experiment: retraining MalwareGAT with one family excluded entirely from train/validation/test, then evaluating only on its untouched samples. Holding out \ZDOneFamily{} (\ZDOneN{} samples) gave a \ZDOneCatch{} zero-day catch rate at \ZDOneConf{} mean confidence, \ZDOneTrigger{} of samples triggering Agent 2; holding out \ZDTwoFamily{} (\ZDTwoN{} samples) gave \ZDTwoCatch{} at \ZDTwoConf{} confidence, confidently but incorrectly attributed to \ZDTwoNearest{}, its nearest known family. The malicious/benign boundary thus generalizes past the training classes -- via similarity to the nearest known malicious cluster, not a dedicated novelty detector -- and Agent 2's gate engages precisely when that generalization is uncertain rather than confidently wrong.

Pooling all ten held-out runs (\EscTotalN{} predictions: \EscBenignN{} benign, \EscKnownMalN{} known-malicious, \EscZeroDayN{} zero-day) through the deployed fusion pipeline, the residual errors are almost entirely novel malware labelled \emph{benign}. Applying (\ref{eq:failclosed}) at $\tau = \GateTau$ raises zero-day recall \EscZDBefore{}$\rightarrow$\EscZDAfter{} and known-malware detection \EscKnownBefore{}$\rightarrow$\EscKnownAfter{}, for \EscFPAfter{} benign false positives (from \EscFPBefore{}). The gain concentrates where it should: across \NovelN{} samples from \NovelFamilies{} unseen families recall is already \NovelBefore{} pre-escalation and unchanged after, because the residual \NovelMissN{} misses are confidently -- not uncertainly -- wrong and never reach the gate at all (Section~\ref{sec:conclusion} examines why). On the \HardN{}-sample subset whose confidence \emph{does} trip the gate, recall instead rises \HardBefore{}$\rightarrow$\HardAfter{} -- the gate had flagged all \HardN{} while the label cleared 39, so the evidence was present and merely unused. Since escalation relabels after fusion and trains nothing, accuracies reported elsewhere are unaffected.

\subsection{Multi-Class Threat Family Evaluation}
To evaluate fine-grained threat discrimination beyond binary classification, MalwareGAT was evaluated across all 11 classes (Benign, Gandcrab, Emotet, Gamarue, Hotbar, DarkKomet, Delf, Domaiq, DLHelper, DriverPack, GameHack), achieving \MulticlassAcc{} overall accuracy and \MulticlassMacroF{} macro F1 (\MulticlassMacroP{} macro precision, \MulticlassMacroR{} macro recall) on the \TestCount{}-sample test set. Precision was consistently high: 100\% on seven of eleven (Delf, Domaiq, DLHelper, DriverPack, Emotet, Gamarue, Gandcrab -- the last at \GandcrabRec{} recall), and \BenignPrec{} on Benign. The lowest was GameHack (\GameHackPrec{}, \GameHackN{} samples) and DarkKomet (\DarkKometPrec{}, \DarkKometN{} samples), both among the smallest-support families.

\section{Conclusion and Future Work}
\label{sec:conclusion}
We presented Agentic-PACX, a hybrid framework fusing a rule-based PAC-X heuristic engine with a structural GAT (MalwareGAT) under trust-based arbitration and dual LLM agents, achieving \BinaryAcc{}/\BinaryPrec{}/\BinaryRec{} binary accuracy, precision and recall, and \MulticlassAcc{} accuracy with \MulticlassMacroF{} macro F1 across 11 families on \TestCount{} held-out samples. Residual errors on unseen families were overwhelmingly novel malware labelled benign -- an $\arg\max$ artifact over trained classes -- so reusing the confidence gate as a fail-closed rule (Section~\ref{sec:failclosed}) converts this into a third \emph{Suspicious} verdict, lifting zero-day recall \EscZDBefore{}$\rightarrow$\EscZDAfter{} over \EscTotalN{} predictions for \EscFPAfter{} false positives, without altering a learned parameter.

Agent 1 bridges the black-box gap via auditable XAI forensic reports; Agent 2 self-heals under pool-diversity and accuracy-regression guardrails that a controlled reproduction confirmed prevent a poorly-formed retrain batch from degrading a converged model. Two limitations remain. Escalation catches only novel malware whose confidence falls below $\tau$, so families placed \emph{confidently} in the wrong class dominate residual misses; the natural fix -- rejecting samples far from every class centroid -- fails, since these samples sit \emph{closer} to a centroid than real known samples do, i.e. genuine feature overlap with benign software, not statistical outlierness, requiring new discriminative features rather than distance-based novelty detection. Second, Agent 2's pool is pseudo-labelled by the model's own unreliable-moment prediction, so labels stay provisional and a collapsed pool is refused. Future work: discriminative features for these families, partial supervision over forced pseudo-labels, production-gateway latency measurement, PAC-X on its native raw-text modality, and federated threat-intelligence sharing across multi-cloud edges.
