...
Existing security systems rely predominantly on static rule-based heuristics or standard machine learning classifiers. Heuristic engines are fast and deterministic but suffer high false-negative rates against obfuscated or zero-day malware; deep learning models often function as black boxes lacking the explainability forensic response needs; and conventional systems treat API requests as flat sequences rather than graphs, missing the structural relationships inherent in advanced threats. This paper proposes Agentic-PACX, an explainable hybrid framework fusing a Prospect-Aspect-Context (PAC-X) heuristic engine with a Graph Attention Network (MalwareGAT) to evaluate both deterministic rule violations and topological anomalies. A dual-pathway fusion layer, orchestrated by cognitive Large Language Model (LLM) agents, provides human-readable forensic reasoning for every detection and gates self-healing transfer-learning loops when confidence drops.
