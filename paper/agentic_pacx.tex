...
\begin{abstract}
Application Programming Interfaces (APIs) are the primary attack surface for distributed cloud microservices, where obfuscation techniques such as dynamic library hooking and API aliasing let attackers evade static rule-based heuristics undetected. Cloud API security demands detecting zero-day threats that static rule signatures miss, while deep learning classifiers remain opaque ``black boxes'' unsuited to security operations center (SOC) forensic response. We propose Agentic-PACX, a hybrid framework fusing a Prospect-Aspect-Context (PAC-X) heuristic engine with a structural Graph Attention Network (MalwareGAT) over a 4-node topological graph, arbitrated by a Decision Fusion Layer under two cognitive LLM agents: Agent 1 generates auditable natural-language XAI forensic reports from trust scores, and Agent 2 autonomously triggers transfer-learning self-healing on low-confidence zero-day detections. On a held-out test set of \TestCount{} real PE-malware graph instances (70/10/20 split, validation-only model selection), MalwareGAT reaches \MulticlassAcc{} accuracy and \MulticlassMacroF{} macro F1 across 11 malware families, and \BinaryAcc{} accuracy, \BinaryPrec{} precision, \BinaryRec{} recall on binary detection. An ablation shows the Fusion Layer safely defers to the structural pathway on this dataset's tabular modality -- its value is arbitration, not a numeric lift -- while PAC-X's interpretability advantage targets raw API-log/text telemetry instead. A leave-one-family-out experiment confirms the malicious/benign boundary generalizes to unseen families (\ZDOneCatch{}--\ZDTwoCatch{} zero-day catch rates), with Agent 2's gate engaging precisely where that generalization is uncertain. Because a novel family cannot be named by an $\arg\max$ over trained classes and is absorbed by the nearest one -- usually benign -- we add a fail-closed rule that escalates an uncertain benign verdict to \emph{Suspicious} rather than clearing it. Measured over \EscTotalN{} pooled predictions, this raises zero-day recall from \EscZDBefore{} to \EscZDAfter{} while holding known-family detection at \EscKnownAfter{}, at a cost of \EscFPAfter{} benign false positives.
\end{abstract}
...
The fused label is an $\arg\max$ over families the network was \emph{trained} on. A genuinely novel family therefore cannot be named: it is absorbed by whichever known class lies nearest in embedding space, and because the benign cluster is the largest and least structurally distinctive, that class is overwhelmingly ``benign''. A zero-day is by definition an attack, so this failure direction is the dangerous one -- the system clears real malware.

Critically, the pipeline is not oblivious in these cases. The same samples carry fused confidence below the Agent 2 gate $\tau$; the uncertainty is measured, and then discarded by a hard $\arg\max$. We therefore treat low confidence as evidence rather than noise, and reuse the gate already defined for Agent 2 as a decision rule, yielding a third verdict state:
\begin{equation}
\label{eq:failclosed}
\hat{y}_{\text{final}} =
\begin{cases}
...
\end{cases}
\end{equation}
where $\tau = \GateTau$ (Section~\ref{sec:agent2}). Three properties are deliberate. First, only a \emph{benign} verdict is escalated: a low-confidence \emph{malicious} verdict is already actioned, so relabelling it would add false alarms while catching nothing new. Second, the escalated state is ``Suspicious'', not ``Malicious'' -- the system has not identified an attack, it has failed to clear the sample, and conflating the two would overstate what was actually detected. Third, this is a labelling policy over the existing pipeline: no weights change and nothing is retrained, so it cannot regress the underlying model. This is the classifier-with-reject-option formulation \cite{b20} composed with the fail-closed principle standard in security decision-making: abstention resolves toward denial rather than admission.
...
\label{sec:agent2}
If fused confidence falls below $\tau = \GateTau$, Agent 2 pools the sample for an active transfer-learning self-healing routine (Algorithm \ref{alg:agent2}). The gate is not a round-number default: it was swept against the true fused-confidence pipeline across all ten leave-one-family-out runs (\EscKnownMalN{} known-malicious, \EscBenignN{} benign, and \EscZeroDayN{} held-out zero-day predictions). At $\tau = \GateTau$ the gate captures \EscZDAfter{} of zero-day samples; beyond $\tau = \EscCliffTau$ the benign false-positive rate rises sharply (to \EscCliffFP{}) for diminishing coverage gains, so $\GateTau$ sits just below that cliff rather than chasing the last points of recall. The same $\tau$ governs the fail-closed rule of Section~\ref{sec:failclosed}, so one threshold carries one meaning -- ``the system cannot vouch for this sample'' -- rather than splitting into two independently drifting tunables.
...
\subsection{Fail-Closed Escalation on Zero-Day Malware}
\label{sec:escresults}
The leave-one-family-out results above expose an asymmetry that a headline catch rate hides: the residual errors are almost entirely novel malware labelled \emph{benign}, the one direction in which a security system must not fail. We therefore evaluated the escalation rule of (\ref{eq:failclosed}) over all ten held-out runs pooled -- \EscTotalN{} predictions in total (\EscBenignN{} benign, \EscKnownMalN{} known-malicious, \EscZeroDayN{} zero-day) -- scoring each sample through the deployed fusion pipeline itself rather than a reimplementation. Table~\ref{tab:escalation} reports the trade-off.

\begin{table}[htbp]
\caption{Effect of fail-closed escalation at $\tau = \GateTau$ (\EscTotalN{} pooled predictions)}
\label{tab:escalation}
\centering
\footnotesize
\begin{tabular}{lccc}
\hline
\textbf{Sample population} & \textbf{Baseline} & \textbf{Escalated} & \textbf{$\Delta$} \\
\hline
Zero-day caught (held-out) & \EscZDBefore{} & \EscZDAfter{} & $+7.09$ \\
Novel families caught & \NovelBefore{} & \NovelAfter{} & $+1.94$ \\
Gate-tripping subset caught & \HardBefore{} & \HardAfter{} & $+69.64$ \\
Known malware caught & \EscKnownBefore{} & \EscKnownAfter{} & $+0.02$ \\
Benign false positives & \EscFPBefore{} & \EscFPAfter{} & $+2.95$ \\
\hline
\end{tabular}
\end{table}

Escalation raises zero-day recall from \EscZDBefore{} to \EscZDAfter{} and leaves known-family detection at \EscKnownAfter{}, at a cost of \EscFPAfter{} benign false positives (from \EscFPBefore{}). The gain concentrates exactly where it should. On \NovelN{} samples spanning \NovelFamilies{} families absent from training, recall moves modestly (\NovelBefore{} to \NovelAfter{}) because most such samples are already confidently flagged. But on the \HardN{}-sample subset drawn from \HardFamilies{} novel families whose fused confidence actually trips the gate -- the population the baseline handles worst -- recall rises from \HardBefore{} to \HardAfter{}: every sample is withheld rather than cleared. Notably, the confidence gate had flagged all \HardN{} of these for Agent 2 while the label cleared 39 of them, confirming that the evidence needed to catch them was present in the pipeline and simply unused.

We report the cost plainly: roughly one benign sample in twenty-eight is now withheld for review rather than cleared. Whether that is acceptable is a deployment decision, not a universal one, and it is the reason $\tau$ is exposed as a tunable rather than fixed. The sharp rise in false positives beyond $\tau = \EscCliffTau$ (to \EscCliffFP{}) marks the practical ceiling. We also note the escalation is a labelling policy applied after fusion: it changes no learned parameters, so the accuracy figures reported elsewhere in this section are unaffected by it.
...
We presented Agentic-PACX, a hybrid threat intelligence framework combining a rule-based Prospect-Aspect-Context (PAC-X) heuristic engine with a structural Graph Attention Network (MalwareGAT), fused under trust-based arbitration and governed by dual cognitive LLM agents. Across \DatasetTotal{} graph instances spanning 11 real-world malware families (\TestCount{} held-out samples, from a 70/10/20 train/validation/test split where validation alone drives model selection), Agentic-PACX achieved \BinaryAcc{} accuracy, \BinaryPrec{} precision, and \BinaryRec{} recall on binary threat detection, and \MulticlassAcc{} accuracy with a \MulticlassMacroF{} macro F1-score on family attribution. An ablation study showed the Decision Fusion Layer safely defers to the structural pathway on this dataset's native tabular modality -- contributing arbitration and safety rather than an accuracy lift -- while the heuristic pathway's designed advantage is expected to materialize on its native raw API-log/text modality. A leave-one-family-out experiment confirmed the model also generalizes to entirely unseen families (\ZDOneCatch{} and \ZDTwoCatch{} zero-day catch rates on two held-out families), with Agent 2's confidence gate engaging where that generalization was uncertain rather than confidently wrong. Analyzing where that generalization still failed showed the residual errors were overwhelmingly novel malware labelled benign -- an artifact of forcing an $\arg\max$ over trained classes onto a family that belongs to none of them. Reusing the confidence gate as a fail-closed decision rule (Section~\ref{sec:failclosed}) converts that measured uncertainty into a third \emph{Suspicious} verdict, lifting zero-day recall from \EscZDBefore{} to \EscZDAfter{} over \EscTotalN{} pooled predictions -- and from \HardBefore{} to \HardAfter{} on the subset the baseline handled worst -- for \EscFPAfter{} benign false positives, without altering a single learned parameter.

Agent 1 bridges the black-box gap by translating edge attention weights into auditable XAI forensic reports; Agent 2 establishes an autonomous self-healing loop, guarded by pool-size, class-diversity, and accuracy-regression checks that a controlled reproduction confirmed prevent exactly the kind of poorly-formed retrain batch that would otherwise silently degrade a converged model. Two limitations bound these claims. First, escalation catches only novel malware whose confidence actually falls below $\tau$; families the network places confidently in the wrong class are unaffected, and these dominate the residual misses on the novel-family set. Confident errors on unseen classes are a representational gap that a threshold cannot close, and addressing them calls for explicit novelty detection rather than a calibration fix. Second, Agent 2's pool is pseudo-labelled by the model's own prediction at precisely the moment that prediction is unreliable; for a genuinely novel family no label over the trained classes is correct, so pool labels are retained as provisional and a pool that has collapsed onto one class is refused for retraining. Future work will therefore add open-set novelty detection alongside the confidence gate, replace pseudo-labelling with partial supervision that does not force a wrong family label, deploy Agentic-PACX on production cloud API gateways to measure real-time latency, validate the PAC-X pathway on its native raw-text modality, and expand federated threat-intelligence sharing across multi-cloud edge environments.
