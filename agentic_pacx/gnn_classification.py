...
if __name__ == "__main__":
    ...
    # Load the actual real dataset generated by scripts/build_graph_dataset.py
    # and normalized by core/dataset_manager.py.
    try:
        full_dataset = torch.load("data/pyg_dataset_norm.pt", weights_only=False)
    ...
    # Splits come from core/dataset_manager.py, computed once and reused here --
    # NOT recomputed in-line. Critically, the model-selection loop below trains
    # against `val_loader` (val_indices.npy), never `test_loader`. The held-out
    # test set is touched exactly once, after training is finished and the best
    # checkpoint is already selected, so the reported test number isn't the
    # best-of-N-epochs score on the set it's graded against.
    try:
        train_indices = np.load(os.path.join("data", "train_indices.npy"))
        val_indices = np.load(os.path.join("data", "val_indices.npy"))
        test_indices = np.load(os.path.join("data", "test_indices.npy"))
    except Exception as e:
        print(
            f"[-] FATAL: Could not load train/val/test indices ({e}). "
            f"Run `python -m core.dataset_manager` first to generate them."
        )
        exit(1)

    train_dataset = [full_dataset[int(i)] for i in train_indices]
    ...
    # Num node features is 260 from scripts/build_graph_dataset.py
    ...
    # Final test-set metrics: computed exactly once, after the checkpoint was
    # already chosen on val_loader. This is the number that goes in the paper.
    final_metrics = evaluate_model(model, test_loader, criterion, device)
    print(
        f"[+] Final Test Metrics (held-out, never used for model selection) | "
    ...
    with open("training_history.json", "w") as f:
        json.dump({
            ...
            "final_test_metrics": {
                ...
                "f1_macro": round(final_metrics["f1_macro"] * 100, 2),
